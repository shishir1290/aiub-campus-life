...
const ContactForm = () => {
  ...
  const validate = () => {
    ...
    else if (!/^\d{2}-\d{5}-\d{1}$/.test(formData.id)) newErrors.id = 'ID must be in the format **-*****-*';
  ...
  return (
    <section className="contact" id="contact">
      ...
      <div className="text-2xl text-blue-600 ml-96 mb-4 ">
          <p className="p-4">Contact Us <hr className="w-36  border-2 border-blue-600 rounded-xl " /></p>
        ...
        <div className="flex justify-center">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-lg">
            ...
            {status && (
              <p className={`mt-4 text-center font-bold ${status.includes('Error') ? 'text-red-500' : 'text-green-500'}`}>
                {status}
              </p>
            )}
          </div>
